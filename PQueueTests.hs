...
validSpan2 :: [Int] -> Bool
validSpan2 xs = case Min.span even (Min.fromList xs) of
	(xs1, q') -> (xs1, Min.toAscList q') == List.span even (List.sort xs)
...
validMapEither xs = Min.mapEither f q == (Min.mapMaybe (either Just (const Nothing) . f) q,
						Min.mapMaybe (either (const Nothing) Just . f) q)
	where	f x	| even x	= Left (x `quot` 2)
			| otherwise	= Right (3 * x + 1)
		q = Min.fromList xs

validMap :: [Int] -> Bool
validMap xs = Min.map f (Min.fromList xs) == Min.fromList (List.map f xs)
	where	f x	| even x	= x `quot` 2
			| otherwise	= 3 * x + 1

validSize :: [Int] -> Bool
validSize xs = Min.size q == List.length xs'
	where	q = Min.drop 10 (Min.fromList xs)
		xs' = List.drop 10 (List.sort xs)

validNull :: Int -> [Int] -> Bool
validNull n xs = Min.null q == List.null xs'
	where	q = Min.drop n (Min.fromList xs)
...
main = do	myCheck validMinToAscList
