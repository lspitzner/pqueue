...
function build-one {
  local ATTRPATH=$1
  # nix-build --no-out-link nix/all.nix -A "\"$ATTRPATH\".pqueue.components.library"\
  #   2> >(tee "$OUTDIR/$ATTRPATH-1-build-lib.txt" >&2)
  # (($? == 0)) || { echo "$ATTRPATH: build src failed" >> "$SUMMARY"; return 1; }
  # nix-build --no-out-link nix/all.nix -A "\"$ATTRPATH\".pqueue.components.tests"\
  #   2> >(tee "$OUTDIR/$ATTRPATH-2-build-test.txt" >&2)
  # (($? == 0)) || { echo "$ATTRPATH: build test failed" >> "$SUMMARY"; return 1; }
  OUT=$(nix-build -o "$OUTDIR/$ATTRPATH-test-result.txt" nix/all.nix -A "\"$ATTRPATH\".pqueue.checks.test"\
    2> >(tee "$OUTDIR/$ATTRPATH-build.txt" >&2))
  (($? == 0)) || { echo "$ATTRPATH: run test failed" >> "$SUMMARY"; return 1; }
  echo "$ATTRPATH: $(tail -n1 "$OUTDIR/$ATTRPATH-test-result.txt")" >> "$SUMMARY"
}
